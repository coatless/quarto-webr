...
- Pinned `engine: knitr` in the deployment templates and the documentation examples. Quarto 1.9.19 widened the scan it uses to work out which languages a document contains ([quarto-cli#13838](https://github.com/quarto-dev/quarto-cli/pull/13838)); from that release on, a document holding a `{webr-r}` cell but naming no engine resolved to `jupyter` and would not render without a Python installation. Nothing about the extension changed - if you hit this in your own document, adding `engine: knitr` to the header is the fix, as [Your first webR-powered Quarto document](qwebr-first-steps.qmd) has always described.

# 0.4.4-dev.1: ??? ??? (??-??-????)

## Features

- Upgraded the embedded version of webR to v0.5.7. ([#236](https://github.com/coatless/quarto-webr/pulls/236))

# 0.4.3: Bumpity Bump (06-24-2025)

## Features

- Upgraded the embedded version of webR to v0.5.2 (R 4.5.1). ([#232](https://github.com/coatless/quarto-webr/pulls/232))
