...
Here, the `fig-width: 5` and `fig-height: 5` option is utilized to set the width of the bar plot.
...
The `out-width` and `out-height` options control physical space the plot will reside in. Let's revisit our previous example and constrain the output area by specifying `out-width: 500px` and `out-height: 400px`.
