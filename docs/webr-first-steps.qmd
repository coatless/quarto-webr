...
For your next steps consider looking at different use cases for [interactive options](webr-code-cell-demos.qmd).

If you're eager to delve even deeper, explore topics like [setting document-level customizations inside the document header](webr-meta-options.qmd), [using R packages inside webR](webr-using-r-packages.qmd), or [hiding and executing code](webr-internal-cell.qmd). These resources will further empower you to make the most of `quarto-webr` and `webR` to elevate your Quarto documents to new heights.