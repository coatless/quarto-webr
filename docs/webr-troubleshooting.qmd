...
If the issue persists, specify the worker files' location using the [`service-worker-url` option](webr-meta-options.qmd) in the document's YAML header or set the [`channel-type` option to `"post-message"`](webr-communication-channels.qmd#sec-post-message).
...
If your directory is missing the `_extensions/` folder, please install the `webr` extension by following the instructions in [Install the quarto-webr extension](webr-first-steps.qmd).
...
By configuring these headers, you enforce a same-origin policy for cross-origin windows and ensure that cross-origin iframes are required to have a COEP policy that allows cross-origin embedding. For more detailed instructions on setting up these headers, please refer to the [`"shared-array-buffer"`](webr-communication-channels.qmd#sec-shared-array-buffer) channel type documentation.
